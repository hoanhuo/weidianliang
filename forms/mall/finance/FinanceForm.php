...
class FinanceForm extends Model
{
    ...
    public function search()
    {
        ...
        $tempQuery = (new Query())->from(['q' => $query]);
            ...
            try {
                if ($model['key'] == 'mch') {
                    $queryList[$model['key']] = $model['class']::find()
                        ...
                        ->leftjoin(
                            ["u" => User::tableName()],
                            "u.id = {$model['user_alias']}.user_id"
                        )
                        ->leftjoin(
                            ['ui' => UserInfo::tableName()],
                            "ui.user_id = {$model['user_alias']}.user_id"
                        );
                    ...
                    if ($this->keyword && empty($this->user_id)) {
                        ...
                        $subOtherQuery = $model['user_class']::find()
                            ->select('user_id')
                            ->where(['like', 'realname', $this->keyword]);
                        $queryList[$model['key']] = $queryList[$model['key']]
                            ->andWhere([
                                'or',
                                ['in', "ui.user_id", $subQuery],
                                ['in', "ui.user_id", $subOtherQuery]
                            ]);
                    }
                    ...
                    $queryList[$model['key']] = $model['class']::find()
                        ...
                        ->leftjoin(
                            ["u" => User::tableName()],
                            "u.id = {$model['key']}.user_id"
                        )
                        ->leftjoin(
                            ['ui' => UserInfo::tableName()],
                            "ui.user_id = {$model['key']}.user_id"
                        )
                        ->leftJoin(
                            ["{$model['user_alias']}" => $model['user_class']::tableName()],
                    ...
                    if ($this->keyword && empty($this->user_id)) {
                        ...
                        $subOtherQuery = $model['user_class']::find()
                            ->select('user_id')
                            ->where(['like', 'name', $this->keyword]);
                        $queryList[$model['key']] = $queryList[$model['key']]
                            ->andWhere([
                                'or',
                                ['in', "{$model['key']}.user_id", $subQuery],
                                ['in', "{$model['key']}.user_id", $subOtherQuery]
                            ]);
                    }
                }
            } catch (\Exception $exception) {
