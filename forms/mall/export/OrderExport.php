...
class OrderExport extends BaseExport
{
    ...
    public function export($query)
    {
        ...
        $list = $query->with(['user.userInfo', 'clerk', 'store', 'detail.goods.goodsWarehouse', 'refund', 'paymentOrder.paymentOrderUnion', 'detail.expressRelation.orderExpress', 'detailExpress'])
    ...
    protected function transform($list)
    {
        ...
        foreach ($this->fieldsKeyList as $item) {
            if (in_array($item, ['goods_name', 'attr', 'goods_num', 'goods_no', 'cost_price', 'send_type', 'express', 'express_no'])) {
                $sign = true;
                break;
            }
        }
