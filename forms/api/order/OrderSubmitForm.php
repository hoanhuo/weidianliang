...
class OrderSubmitForm extends Model
{
    ...
    public function getAllData()
    {
        ...
        foreach ($listData as &$mchItem) {
            if (empty($mchItem['insert_rows']) || !is_array($mchItem['insert_rows'])) continue;
    ...
    protected function setCouponDiscountData($mchItem, $formMchItem)
    {
        ...
        if ($totalGoodsPrice <= 0) { // 价格已优惠到0不再使用优惠券
            $mchItem['coupon']['coupon_error'] = '无法使用优惠券';
            return $mchItem;
        }
    ...
    protected function setIntegralDiscountData($mchItem, $formMchItem)
    {
        ...
        foreach ($mchItem['goods_list'] as &$goodsItem) {
            if (!$this->isGoodsEnableIntegral($goodsItem)) continue;
    ...
    {
        /** @var array 没有被包邮的商品列表（未单独设置包邮规则） */
        $noZeroGoodsList = [];

        /** @var array 没有被包邮的单独设置包邮规则的商品列表（有单独设置包邮规则，但不满足） */
        $noZeroIndRuleGoodsList = [];

        /** @var array 被包邮的单独设置包邮规则的商品列表 */
        $zeroIndRuleGoodsList = [];
        ...
        foreach ($mchItem['goods_list'] as $goodsItem) {
            /** @var boolean 商品有设置单品包邮规则 */
            $_hasRule = false;

            /** @var boolean 商品设置了包邮规则并且满足 */
            $_inRule = false;
            ...
            if (is_numeric($goodsItem['forehead']) && $goodsItem['forehead'] > 0) { // 单品设置了满额包邮
                $_hasRule = true;
        ...
        foreach ($allList as &$userCoupon) {
            ...
            if ($userCoupon->coupon->appoint_type == 2) {
                ...
                if (count($goodsWarehouseList)) {
                    ...
                    if ($isCantUse) $cantUseList[] = $userCoupon;
                    continue;
                }
    ...
    {
        if (empty($mchItem['goods_list'])) return false;
        $isECard = false;
        foreach ($mchItem['goods_list'] as $goods) {
            if (empty($goods['goodsWarehouse'])) continue;
            $goodsWarehouse = $goods['goodsWarehouse'];
