...
class OrderExpressForm extends Model
{
    ...
    public function search()
    {
        ...
        } catch (HttpException | WdException $exception) {
            return [
    ...
    {
        $mall = (new Mall())->getMallSetting(['kdniao_mch_id', 'kdniao_api_key', 'express_select_type', 'express_aliapy_code']);
        if(empty($mall)) {
            return ['', '', '', ''];
        }
        $mch_id = $mall['kdniao_mch_id'];
        $api_key = $mall['kdniao_api_key'];
        return [$mch_id, $api_key, $mall['express_aliapy_code'], $mall['express_select_type']];
    }
