...
class OrderController extends Controller
{
    ...
    {
        $pintuanOrderIds = PintuanOrderRelation::find()->andWhere(['cancel_status' => 1])
            ->andWhere(['>', 'created_at', '2020-05-31 00:00:00'])->select('pintuan_order_id');

        $list = PintuanOrders::find()->andWhere(['mall_id' => \Yii::$app->mall->id, 'id' => $pintuanOrderIds, 'status' => 2])->with('orderRelation.order.detail', 'orderRelation.user')->all();

        $newList = [];
        foreach ($list as $key => $value) {
            foreach ($value->orderRelation as $key2 => $orItem) {
                if ($orItem->order->is_pay == 1 && $orItem->order->cancel_status = 0) {
                    $newItem = [];
                    $newItem['pintuan_order_id'] = $value->id;
                    $newItem['order_no'] = $orItem->order->order_no;
                    $newItem['user_id'] = $orItem->order->user_id;
                    $newItem['nickname'] = $orItem->order->user->nickname;
                    $newItem['cancel_status'] = $orItem->order->cancel_status;
                    $detailList = [];
                    foreach ($orItem->order->detail as $key => $dItem) {
                        $detailItem = [];
                        $detailItem['goods_id'] = $dItem->goods_id;
                        $detailItem['num'] = $dItem->num;
                        $detailList[] = $detailItem;
                    }
                    $newItem['order_detail'] = $detailList;
                    $newList[] = $newItem;
                }
            }
        }
