...
class GoodsForm extends Model
{
    ...
    public function getList()
    {
        ...
        foreach ($list as $item) {
            ...
            $newItem['level_price'] = CommonGoodsMember::getCommon()->getGoodsMemberPrice($item);
    ...
    public function detail()
    {
        try {
            ...
            foreach ($goodsList as $gItem) {
                $form->goods = $gItem;
                $ptGoods = $form->getAll(['attr', 'price_min', 'price_max']);

                $groupsPriceMax = $groupsPriceMax == 0 ? $ptGoods['price_max'] : max($ptGoods['price_max'], $groupsPriceMax);
                $groupsPriceMin = $groupsPriceMin == 0 ? $ptGoods['price_min'] : max($ptGoods['price_min'], $groupsPriceMin);
                $memberPriceMin = 0;
                $memberPriceMax = 0;
