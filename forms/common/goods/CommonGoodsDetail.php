...
class CommonGoodsDetail extends Model
{
    ...
    {
        $goods = Goods::find()->with(['attr', 'share', 'cards', 'goodsCardRelation.goodsCards', 'services', 'goodsWarehouse.cats'])
            ->where([
                'is_delete' => 0,
                'id' => $id,
    ...
    public function getAll($params = [], $ignore = [])
    {
        ...
        $this->setOffer($this->goods->goodsWarehouse->type != 'ecard');
        $this->setShipping($this->goods->goodsWarehouse->type != 'ecard');
        $this->setIsLimit($this->goods->goodsWarehouse->type != 'ecard');
    ...
    private function getDefault()
    {
        return [
            ...
            'goods_marketing_award', 'vip_card_appoint', 'plugin_extra'
        ];
    }
    ...
    public function getExpressPrice()
    {
        ...
        if ($this->goods->goodsWarehouse->type == 'ecard') {
            return '';
        }
