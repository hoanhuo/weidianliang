                ...
                setTimeout(() => {
                    ...
                    request({
                        ...
                        if (e.data.code === 0) {
                            ...
                            let cash_map = e.data.data.cash_map;
                            let title = '包括';
                            if (cash_map.indexOf('mch') !== -1) {
                                title += '多商户提现、'
                            }
                            if (cash_map.indexOf('share') !== -1) {
                                title += '分销商提现、'
                            }
                            if (cash_map.indexOf('bonus') !== -1) {
                                title += '团队分红、'
                            }
                            if (cash_map.indexOf('stock') !== -1) {
                                title += '股东分红提现、'
                            }
                            if (cash_map.indexOf('region') !== -1) {
                                title += '区域代理提现、'
                            }
                            title = title.slice(0, title.length - 1);
                            this.cash_price_desc = title === '包' ? '' : title;
                        }
                    }).catch(e => {
                        this.loading = false;
                    });
