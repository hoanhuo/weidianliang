...
<template id="diy-module">
    <div class="diy-module">
        <div class="diy-component-preview">
            <div class="pane-second" v-if="value.list && value.list.length" style="position: relative">
                <div class="scroll"
                     v-if="value.list.length > 1"
                     :style="{backgroundColor: value.tabBackground}">
                    <div flex="dir:left" style="height: 100%;position: relative">
                        <div v-for="(item,index) in value.list" :index="index"
                            ...
                            <div :style="[tabNameFill(index)]">
                                ...
                                <div v-if="value.tabType === 'line'" class="line"
                                     :style="{background: current === index ? value.tabColor : 'none'}"
                                ></div>
                            </div>
                        </div>
                        <div v-if="value.list && value.list.length > 4"
                             style="display: flex;box-grow: 0,width: 20px;height: 100%"></div>
                    </div>
                </div>
                <div v-if="false && value.list.length > 4" :style="{background: data.tabBackground}" class="scroll-right"></div>
                <div v-for="(component, index) in value.list[current]['data']" :key="component.key">
            ...
            <el-form label-width="120px">
                <template v-if="data.list && data.list.length > 1">
                    ...
                    <el-form-item label="自定义模块">
                        <draggable style="width: 355px" flex="dir:top" v-model="data.list"
